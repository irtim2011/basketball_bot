...
def poll_text(start, answer=None, end=None):
    ...
    if answer == 'yes':
        return text + '\n✅ Ты в составе! Ждём на площадке 💪🏀\nОтвет можно изменить в любой момент этими кнопками. Таблица обновится автоматически.'
    return text + '\n❌ Записал: не придёшь. Увидимся на следующей тренировке! 🏀\nЕсли планы изменятся — нажми «Приду». Ответ можно изменить в любой момент.'

def reminder_label():
    if POLL_OFFSET_MINUTES == 1440:
        return 'за 24 часа'
